...
namespace Microsoft.Xna.Framework {
    static class SDL2_FNAPlatform {
        #region Static Constants

        static string OSVersion;

        static readonly bool UseScancodes = Environment.GetEnvironmentVariable(
            "FNA_KEYBOARD_USE_SCANCODES"
            ) == "1";

        static bool SupportsGlobalMouse;

        static float HapticMaxWorkaround;
        ...
        static readonly List<Game> activeGames = new List<Game>();
        ...
        public static string ProgramInit() {
            ...
            var hint = SDL.SDL_GetHint(SDL.SDL_HINT_JOYSTICK_ALLOW_BACKGROUND_EVENTS);
        ...
        public static GameWindow CreateWindow() {
            ...
            var forceES3 = Environment.GetEnvironmentVariable(
                "FNA_OPENGL_FORCE_ES3"
                ) == "1";
            var forceCoreProfile = Environment.GetEnvironmentVariable(
                "FNA_OPENGL_FORCE_CORE_PROFILE"
                ) == "1";
            var forceCompatProfile = Environment.GetEnvironmentVariable(
...
#endif
            var title = AssemblyHelper.GetDefaultWindowTitle();
        ...
        public static void ApplyWindowChanges(
            ...
            if (center) {
                var pos = SDL.SDL_WINDOWPOS_CENTERED_DISPLAY(displayIndex);
        ...
        static void INTERNAL_SetIcon(IntPtr window, string title) {
            ...
            try {
                ...
                if (!String.IsNullOrEmpty(fileIn)) {
                    var icon = SDL_image.IMG_Load(fileIn);
            ...
            if (!String.IsNullOrEmpty(fileIn)) {
                var icon = SDL.SDL_LoadBMP(fileIn);
                SDL.SDL_SetWindowIcon(window, icon);
                SDL.SDL_FreeSurface(icon);
            }
        }

        static string INTERNAL_GetIconName(string title) {
            var fileIn = Path.Combine(TitleLocation.Path, title);
        ...
        public static void RunLoop(Game game) {
            ...
            var displayIndex = SDL.SDL_GetWindowDisplayIndex(
                game.Window.Handle
                );
            ...
            if (OSVersion.Equals("Mac OS X")) {
                var hint = SDL.SDL_GetHint(SDL.SDL_HINT_VIDEO_MAC_FULLSCREEN_SPACES);
			 ...
			 */
            var textInputCharacters = new char[] {
                (char)2, // Home
                (char)3, // End
                (char)8, // Backspace
                (char)9, // Tab
            ...
            var textInputBindings = new Dictionary<Keys, int>() {
                { Keys.Home, 0 },
                { Keys.End, 1 },
                { Keys.Back, 2 },
                { Keys.Tab, 3 },
            ...
            var textInputControlDown = new bool[textInputCharacters.Length];
            var textInputControlRepeat = new int[textInputCharacters.Length];
            var textInputSuppress = false;
            ...
            while (game.RunApplication) {
                while (SDL.SDL_PollEvent(out evt) == 1) {
                    ...
                    if (evt.type == SDL.SDL_EventType.SDL_KEYDOWN) {
                        ...
                        if (evt.window.windowEvent == SDL.SDL_WindowEventID.SDL_WINDOWEVENT_FOCUS_GAINED) {
							 ...
							 * -flibit
							 */
                            var newIndex = SDL.SDL_GetWindowDisplayIndex(
                                game.Window.Handle
                                );
                ...
                for (var i = 0; i < textInputCharacters.Length; i += 1) {
        ...
        public static GraphicsAdapter[] GetGraphicsAdapters() {
            var filler = new SDL.SDL_DisplayMode();
            var adapters = new GraphicsAdapter[SDL.SDL_GetNumVideoDisplays()];
            for (var i = 0; i < adapters.Length; i += 1) {
                var modes = new List<DisplayMode>();
                var numModes = SDL.SDL_GetNumDisplayModes(i);
        ...
        static string GetBaseDirectory() {
            ...
            var result = SDL.SDL_GetBasePath();
        ...
        static string MonoPathRootWorkaround(string storageRoot) {
        ...
        public static void TextureDataFromStream(
            ...
            var surface = SDL_image.IMG_Load_RW(
            ...
            if (reqWidth != -1 && reqHeight != -1) {
                ...
                // Alloc surface, blit!
                var newSurface = SDL.SDL_CreateRGBSurface(
                    0,
                    resultWidth,
                    resultHeight,
                    32,
            ...
            unsafe {
                ...
                if ((pixels == null) || (pixels.Length < pixelsLength))
                    pixels = new byte[pixelsLength]; // MUST be SurfaceFormat.Color!
                Marshal.Copy(surPtr->pixels, pixels, 0, pixelsLength);
            }
            SDL.SDL_FreeSurface(surface);
        ...
        public static void SavePNG(
            ...
            var surface = INTERNAL_getScaledSurface(
                data,
                imgWidth,
                imgHeight,
                width,
        ...
        public static void SaveJPG(
            ...
            var temp = SDL.SDL_ConvertSurfaceFormat(
                surface,
        ...
        public static IntPtr INTERNAL_getScaledSurface(
            ...
            var surface = SDL.SDL_CreateRGBSurface(
                0,
                srcW,
                srcH,
                32,
            ...
            if (srcW != dstW || srcH != dstH) {
                var scaledSurface = SDL.SDL_CreateRGBSurface(
                    0,
                    dstW,
                    dstH,
                    32,
        ...
        static unsafe IntPtr INTERNAL_convertSurfaceFormat(IntPtr surface) {
            var result = surface;
            unsafe {
                var surPtr = (SDL.SDL_Surface*)surface;
        ...
        static class FakeRWops {
            [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
            delegate long SizeFunc(IntPtr context);

            [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
            delegate long SeekFunc(
                IntPtr context,
                long offset,
                int whence
                );
            ...
            [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
            delegate int CloseFunc(IntPtr context);
            ...
            static extern IntPtr SDL_AllocRW();

            [DllImport("SDL2", CallingConvention = CallingConvention.Cdecl)]
            static extern void SDL_FreeRW(IntPtr area);

            static readonly Dictionary<IntPtr, Stream> streamMap =
                new Dictionary<IntPtr, Stream>();

            // Based on PNG_ZBUF_SIZE default
            static byte[] temp = new byte[8192];

            static readonly SizeFunc sizeFunc = size;
            static readonly SeekFunc seekFunc = seek;
            static readonly ReadFunc readFunc = read;
            static readonly WriteFunc writeFunc = write;
            static readonly CloseFunc closeFunc = close;

            static readonly IntPtr sizePtr =
                Marshal.GetFunctionPointerForDelegate(sizeFunc);

            static readonly IntPtr seekPtr =
                Marshal.GetFunctionPointerForDelegate(seekFunc);

            static readonly IntPtr readPtr =
                Marshal.GetFunctionPointerForDelegate(readFunc);

            static readonly IntPtr writePtr =
                Marshal.GetFunctionPointerForDelegate(writeFunc);

            static readonly IntPtr closePtr =
                Marshal.GetFunctionPointerForDelegate(closeFunc);

            public static IntPtr Alloc(Stream stream) {
                if (stream ==  null)
                    throw new ArgumentNullException("stream");
                var rwops = SDL_AllocRW();
                unsafe {
                    var p = (PartialRWops*)rwops;
                    p->size = sizePtr;
                    p->seek = seekPtr;
                    p->read = readPtr;
                    p->write = writePtr;
            ...
            [MonoPInvokeCallback(typeof(SizeFunc))]
            static long size(IntPtr context) {
                return -1;
            }

            [MonoPInvokeCallback(typeof(SeekFunc))]
            static long seek(IntPtr context, long offset, int whence) {
                Stream stream;
                lock (streamMap) {
            ...
            [MonoPInvokeCallback(typeof(ReadFunc))]
            static IntPtr read(
                IntPtr context,
                IntPtr ptr,
                IntPtr size,
                ...
                Stream stream;
                var len = size.ToInt32() * maxnum.ToInt32();
                lock (streamMap) {
                    stream = streamMap[context];
            ...
            [MonoPInvokeCallback(typeof(WriteFunc))]
            static IntPtr write(
                IntPtr context,
                IntPtr ptr,
                IntPtr size,
                ...
                Stream stream;
                var len = size.ToInt32() * num.ToInt32();
                lock (streamMap) {
                    stream = streamMap[context];
            ...
            [MonoPInvokeCallback(typeof(CloseFunc))]
        ...
        public static Microphone[] GetMicrophones() {
            ...
            var numDev = SDL.SDL_GetNumAudioDevices(1);
            if (numDev < 1) {
                // Blech
                return new Microphone[0];
            }
            var result = new Microphone[numDev + 1];
            ...
            for (var i = 0; i < numDev; i += 1) {
                var name = SDL.SDL_GetAudioDeviceName(i, 1);
        ...
        static readonly IntPtr[] INTERNAL_devices = new IntPtr[GamePad.GAMEPAD_COUNT];
        static readonly Dictionary<int, int> INTERNAL_instanceList = new Dictionary<int, int>();
        static readonly string[] INTERNAL_guids = GenStringArray();

        // Haptic device information
        static readonly IntPtr[] INTERNAL_haptics = new IntPtr[GamePad.GAMEPAD_COUNT];
        static readonly HapticType[] INTERNAL_hapticTypes = new HapticType[GamePad.GAMEPAD_COUNT];

        // Light bar information
        static readonly string[] INTERNAL_lightBars = GenStringArray();

        // Cached GamePadStates/Capabilities
        static readonly GamePadState[] INTERNAL_states = new GamePadState[GamePad.GAMEPAD_COUNT];
        static readonly GamePadCapabilities[] INTERNAL_capabilities = new GamePadCapabilities[GamePad.GAMEPAD_COUNT];

        // We use this to apply XInput-like rumble effects.
        static SDL.SDL_HapticEffect INTERNAL_leftRightEffect = new SDL.SDL_HapticEffect {
        ...
        static readonly ushort[] leftRightMacHackData = { 0, 0 };
        static GCHandle leftRightMacHackPArry = GCHandle.Alloc(leftRightMacHackData, GCHandleType.Pinned);
        static readonly IntPtr leftRightMacHackPtr = leftRightMacHackPArry.AddrOfPinnedObject();

        static SDL.SDL_HapticEffect INTERNAL_leftRightMacHackEffect = new SDL.SDL_HapticEffect {
        ...
        static readonly GamePadType[] INTERNAL_gamepadType = new GamePadType[] {
            GamePadType.Unknown,
            GamePadType.GamePad,
            GamePadType.Wheel,
        ...
        public static GamePadState GetGamePadState(int index, GamePadDeadZone deadZoneMode) {
            var device = INTERNAL_devices[index];
            if (device == IntPtr.Zero) {
                return new GamePadState();
            }

            // The "master" button state is built from this.
            var gc_buttonState = (Buttons)0;
            ...
            // Triggers
            var triggerLeft = (float)SDL.SDL_GameControllerGetAxis(
                device,
                SDL.SDL_GameControllerAxis.SDL_CONTROLLER_AXIS_TRIGGERLEFT
                ) / 32767.0f;
            var triggerRight = (float)SDL.SDL_GameControllerGetAxis(
                device,
        ...
        public static bool SetGamePadVibration(int index, float leftMotor, float rightMotor) {
            var haptic = INTERNAL_haptics[index];
            var type = INTERNAL_hapticTypes[index];
        ...
        public static void SetGamePadLightBar(int index, Color color) {
            ...
            var baseDir = INTERNAL_lightBars[index];
            try {
        ...
        static void INTERNAL_AddInstance(int dev) {
            var which = -1;
            for (var i = 0; i < INTERNAL_devices.Length; i += 1) {
            ...
            var thisJoystick = SDL.SDL_GameControllerGetJoystick(INTERNAL_devices[which]);

            // Pair up the instance ID to the player index.
            // FIXME: Remove check after 2.0.4? -flibit
            var thisInstance = SDL.SDL_JoystickInstanceID(thisJoystick);
			 ...
			 */
            var vendor = SDL.SDL_JoystickGetVendor(thisJoystick);
            var product = SDL.SDL_JoystickGetProduct(thisJoystick);
            ...
            if (OSVersion.Equals("Linux") &&
                ...
                var dirs = Directory.GetDirectories("/sys/class/leds/");
                ...
                var numLights = 0;
                for (var i = 0; i < INTERNAL_lightBars.Length; i += 1) {
        ...
        static void INTERNAL_RemoveInstance(int dev) {
            int output;
        ...
        static Buttons READ_StickToButtons(Vector2 stick, Buttons left, Buttons right, Buttons up, Buttons down, float DeadZoneSize) {
            var b = (Buttons)0;
        ...
        static string[] GenStringArray() {
            var result = new string[GamePad.GAMEPAD_COUNT];
        ...
        public static TouchPanelCapabilities GetTouchCapabilities() {
            var touchDeviceExists = SDL.SDL_GetNumTouchDevices() > 0;
        ...
        static readonly Dictionary<int, Keys> INTERNAL_keyMap = new Dictionary<int, Keys>() {
        ...
        static readonly Dictionary<int, Keys> INTERNAL_scanMap = new Dictionary<int, Keys>() {
        ...
        static readonly Dictionary<int, SDL.SDL_Scancode> INTERNAL_xnaMap = new Dictionary<int, SDL.SDL_Scancode>() {
        ...
        static Keys ToXNAKey(ref SDL.SDL_Keysym key) {
            Keys retVal;
            if (UseScancodes) {
        ...
        public static Keys GetKeyFromScancode(Keys scancode) {
            ...
            if (INTERNAL_xnaMap.TryGetValue((int)scancode, out retVal)) {
                Keys result;
                var sym = SDL.SDL_GetKeyFromScancode(retVal);
        ...
        static readonly SDL.SDL_EventFilter win32OnPaint = Win32OnPaint;

        static unsafe int Win32OnPaint(IntPtr func, IntPtr evtPtr) {
